...
try {
    ...
    app.listen(port, function() {
        if (process.env.APP_DEBUG) {
            console.log(new Date, '[Redis]Server is running on port ' + port);
        }
    });
    ...
    try {
        ...
        try {
            ...
            console.log(new Date, '[Redis]Cannot connect to Redis server on ' + port);
            process.exit(1);
        }
    } catch (err) {
    ...
    console.log(new Date, '[Redis]Server failed to run on port ' + port);
    process.exit(1);
}
